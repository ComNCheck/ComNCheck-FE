import { AnswerRequest } from "./answer.type";
import instance from "./instance";

// MY - 답변하기 post
export const postAnswer = async (data: AnswerRequest): Promise<void> => {
  try {
    await instance.post("/api/v1/major/answers", data);
  } catch (error) {
    console.error("MY - 답변하기 post API 요청 실패:", error);
    throw error;
  }
};

// MY - 답변하기 get
export const getAnswer = async (data: AnswerRequest): Promise<void> => {
  try {
    await instance.post("/api/v1/major/answers/${answerId}", data);
  } catch (error) {
    console.error("MY - 답변하기 get API 요청 실패:", error);
    throw error;
  }
};
