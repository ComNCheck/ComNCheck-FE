...
import { modifyEvent, inquireEvent } from "@/apis/notice";
...
export default function EventModify() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (id) {
        try {
          const eventData = await inquireEvent(Number(id));
          setValues({
  ...
  const handleNextClick = async () => {
    if (isFormValid && id) {
      try {
        ...
        const response = await modifyEvent(formData, Number(id));
