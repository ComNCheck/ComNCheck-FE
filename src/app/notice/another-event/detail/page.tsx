...
import { inquireAnotherEvent } from "@/apis/notice";
...
export default function AnotherEventDetail() {
  ...
  useEffect(() => {
    if (id) {
      inquireAnotherEvent(parseInt(id))
        .then((data) => {
  ...
  const handleWriteClick = () => {
    router.push(`/notice/another-event/modify?id=${event.id}`);
  };
